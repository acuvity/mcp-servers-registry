...
[ -z "$GRAPHLIT_ORGANIZATION_ID" ] && echo "!!! Error mcp-server-graphlit requires GRAPHLIT_ORGANIZATION_ID env var to be set." && exit 1
[ -z "$GRAPHLIT_ENVIRONMENT_ID" ] && echo "!!! Error mcp-server-graphlit requires GRAPHLIT_ENVIRONMENT_ID env var to be set." && exit 1
[ -z "$GRAPHLIT_JWT_SECRET" ] && echo "!!! Error mcp-server-graphlit requires GRAPHLIT_JWT_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- graphlit-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- graphlit-mcp-server
fi
