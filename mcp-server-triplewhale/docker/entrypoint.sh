...
[ -z "$TRIPLEWHALE_API_KEY" ] && echo "!!! Error mcp-server-triplewhale requires TRIPLEWHALE_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-triplewhale "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-triplewhale start $TRIPLEWHALE_API_KEY
fi
