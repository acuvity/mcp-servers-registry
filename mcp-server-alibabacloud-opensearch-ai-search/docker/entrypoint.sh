#!/bin/sh
. .venv/bin/activate
[ -z "$AISEARCH_API_KEY" ] && echo "!!! Error mcp-server-alibabacloud-opensearch-ai-search requires AISEARCH_API_KEY env var to be set." && exit 1
[ -z "$AISEARCH_ENDPOINT" ] && echo "!!! Error mcp-server-alibabacloud-opensearch-ai-search requires AISEARCH_ENDPOINT env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- aisearch-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- aisearch-mcp-server
fi
