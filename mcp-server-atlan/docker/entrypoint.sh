#!/bin/sh
. .venv/bin/activate
[ -z "$ATLAN_API_KEY" ] && echo "!!! Error mcp-server-atlan requires ATLAN_API_KEY env var to be set." && exit 1
[ -z "$ATLAN_BASE_URL" ] && echo "!!! Error mcp-server-atlan requires ATLAN_BASE_URL env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- atlan-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- atlan-mcp-server
fi
