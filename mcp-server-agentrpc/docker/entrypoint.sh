...
[ -z "$AGENTRPC_API_SECRET" ] && echo "!!! Error mcp-server-agentrpc requires AGENTRPC_API_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- agentrpc "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- agentrpc mcp
fi
