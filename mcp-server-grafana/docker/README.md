    ...
    "servers": {
      "acuvity-mcp-server-grafana": {
        ...
        "args": [
          ...
          "-e",
          "GRAFANA_URL",
          "docker.io/acuvity/mcp-server-grafana:v0.3.0"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        ...
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","-e","GRAFANA_API_KEY","-e","GRAFANA_URL","docker.io/acuvity/mcp-server-grafana:v0.3.0"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only -e GRAFANA_API_KEY -e GRAFANA_URL docker.io/acuvity/mcp-server-grafana:v0.3.0`
...
```console
docker run -it -p 8000:8000 --rm --read-only -e GRAFANA_API_KEY -e GRAFANA_URL docker.io/acuvity/mcp-server-grafana:v0.3.0
```
