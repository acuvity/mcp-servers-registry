...
**Maintained by**:
  ...
  - [ grafana ](https://github.com/grafana/mcp-grafana) for original source application
...
**Latest tags:**
  - `latest` -> `1.0.0-v0.4.1` -> `v0.4.1`
  - [older tags](https://hub.docker.com/r/acuvity/mcp-server-grafana/tags)

**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-grafana:latest`
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-grafana:v0.4.1`
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-grafana:1.0.0-v0.4.1`
...
```json
{
  "mcp": {
    "servers": {
      "acuvity-mcp-server-grafana": {
        "env": {
          "GRAFANA_API_KEY": "TO_BE_SET",
          "GRAFANA_URL": "TO_BE_SET"
        },
        "command": "docker",
        "args": [
          "run",
          ...
          "-e",
          "GRAFANA_URL",
          "docker.io/acuvity/mcp-server-grafana:v0.4.1"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        "env": {"GRAFANA_API_KEY":"TO_BE_SET","GRAFANA_URL":"TO_BE_SET"},
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","-e","GRAFANA_API_KEY","-e","GRAFANA_URL","docker.io/acuvity/mcp-server-grafana:v0.4.1"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only -e GRAFANA_API_KEY -e GRAFANA_URL docker.io/acuvity/mcp-server-grafana:v0.4.1`
...
```console
docker run -it -p 8000:8000 --rm --read-only -e GRAFANA_API_KEY -e GRAFANA_URL docker.io/acuvity/mcp-server-grafana:v0.4.1
```
...
```console
helm show readme oci://docker.io/acuvity/mcp-server-grafana --version 1.0.0
````
...
From there your MCP server mcp-server-grafana will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-grafana` on port `8000` by default. You can change that by looking at the `service` section of the `values.yaml` file.

### How to Monitor

The deployment will create a Kubernetes service with a `healthPort`, that is used for liveness probes and readiness probes. This health port can also be used by the monitoring stack of your choice and exposes metrics under the `/metrics` path.

See full charts [Readme](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-grafana/charts/mcp-server-grafana/README.md) for more details about settings and runtime security including guardrails activation.

</details>

# 🧠 Server features

For detailed list of all features, tools, arguments and SBOM hashes provided by this server please consult the [readme](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-grafana)
