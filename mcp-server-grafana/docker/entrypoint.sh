#!/bin/sh
[ -z "$GRAFANA_API_KEY" ] && echo "!!! Error mcp-server-grafana requires GRAFANA_API_KEY env var to be set." && exit 1
[ -z "$GRAFANA_URL" ] && echo "!!! Error mcp-server-grafana requires GRAFANA_URL env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-grafana "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-grafana
fi
