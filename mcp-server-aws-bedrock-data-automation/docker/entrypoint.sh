#!/bin/sh
. .venv/bin/activate
[ -z "$AWS_BUCKET_NAME" ] && echo "!!! Error mcp-server-aws-bedrock-data-automation requires AWS_BUCKET_NAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.aws-bedrock-data-automation-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.aws-bedrock-data-automation-mcp-server
fi
