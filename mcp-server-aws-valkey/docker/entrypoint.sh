...
[ -z "$VALKEY_PWD" ] && echo "!!! Error mcp-server-aws-valkey requires VALKEY_PWD env var to be set." && exit 1
export VALKEY_USE_SSL=${VALKEY_USE_SSL:-"false"}
export VALKEY_CERT_REQS=${VALKEY_CERT_REQS:-"required"}
export VALKEY_CLUSTER_MODE=${VALKEY_CLUSTER_MODE:-"false"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.valkey-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.valkey-mcp-server
fi
