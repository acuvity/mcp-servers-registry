...
[ -z "$APIFY_TOKEN" ] && echo "!!! Error mcp-server-apify-rag-web-browser requires APIFY_TOKEN env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-rag-web-browser "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-rag-web-browser
fi
