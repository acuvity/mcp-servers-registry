...
[ -z "$CHANNEL_ACCESS_TOKEN" ] && echo "!!! Error mcp-server-line-bot requires CHANNEL_ACCESS_TOKEN env var to be set." && exit 1
[ -z "$DESTINATION_USER_ID" ] && echo "!!! Error mcp-server-line-bot requires DESTINATION_USER_ID env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- line-bot-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- line-bot-mcp-server
fi
