...
[ -z "$MVX_NETWORK" ] && echo "!!! Error mcp-server-multiversx requires MVX_NETWORK env var to be set." && exit 1
[ -z "$MVX_WALLET" ] && echo "!!! Error mcp-server-multiversx requires MVX_WALLET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mx-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mx-mcp
fi
