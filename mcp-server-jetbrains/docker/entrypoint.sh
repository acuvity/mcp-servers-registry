...
[ -z "$IDE_PORT" ] && echo "!!! Error mcp-server-jetbrains requires IDE_PORT env var to be set." && exit 1
[ -z "$HOST" ] && echo "!!! Error mcp-server-jetbrains requires HOST env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-jetbrains-proxy "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-jetbrains-proxy
fi
