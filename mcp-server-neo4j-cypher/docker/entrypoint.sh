#!/bin/sh
. .venv/bin/activate
[ -z "$NEO4J_DATABASE" ] && echo "!!! Error mcp-server-neo4j-cypher requires NEO4J_DATABASE env var to be set." && exit 1
[ -z "$NEO4J_PASSWORD" ] && echo "!!! Error mcp-server-neo4j-cypher requires NEO4J_PASSWORD env var to be set." && exit 1
[ -z "$NEO4J_URI" ] && echo "!!! Error mcp-server-neo4j-cypher requires NEO4J_URI env var to be set." && exit 1
[ -z "$NEO4J_USERNAME" ] && echo "!!! Error mcp-server-neo4j-cypher requires NEO4J_USERNAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-neo4j-cypher "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-neo4j-cypher
fi
