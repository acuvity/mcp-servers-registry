...
[![Docker](https://img.shields.io/docker/image-size/acuvity/mcp-server-neo4j-cypher/0.5.2?logo=docker&logoColor=fff&label=0.5.2)](https://hub.docker.com/r/acuvity/mcp-server-neo4j-cypher)
[![PyPI](https://img.shields.io/badge/0.5.2-3775A9?logo=pypi&logoColor=fff&label=mcp-neo4j-cypher)](https://github.com/neo4j-contrib/mcp-neo4j/tree/HEAD/servers/mcp-neo4j-cypher)
...
The [ARC](https://github.com/acuvity/mcp-servers-registry/tree/main) container includes a [built-in Rego policy](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-neo4j-cypher/docker/policy.rego) that enables a set of runtime [guardrails](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-neo4j-cypher#%EF%B8%8F-guardrails) to help enforce security, privacy, and correct usage of your services. Below is list of each guardrail provided.
...
**Resources**:
  - [Charts](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-neo4j-cypher/charts/mcp-server-neo4j-cypher)
  - [Dockerfile](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-neo4j-cypher/docker/Dockerfile)

**Current supported version:**
  - charts: `1.0.0`
  - container: `1.0.0-0.5.2`
...
**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  ...
  - container: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-neo4j-cypher:1.0.0-0.5.2`
...
```console
helm show chart oci://docker.io/acuvity/mcp-server-neo4j-cypher --version 1.0.0
````
...
```console
helm upgrade mcp-server-neo4j-cypher oci://docker.io/acuvity/mcp-server-neo4j-cypher --version 1.0.0
```

Uninstall with helm

```console
helm uninstall mcp-server-neo4j-cypher
```

From there your MCP server mcp-server-neo4j-cypher will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-neo4j-cypher` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-neo4j-cypher
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-neo4j-cypher` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
...
```console
helm upgrade mcp-server-neo4j-cypher oci://docker.io/acuvity/mcp-server-neo4j-cypher --version 1.0.0 --set 'minibridge.guardrails={secrets-redaction}'
```
...
```console
helm upgrade mcp-server-neo4j-cypher oci://docker.io/acuvity/mcp-server-neo4j-cypher --version 1.0.0 -f values.yaml
```

To enable basic auth:

```console
helm upgrade mcp-server-neo4j-cypher oci://docker.io/acuvity/mcp-server-neo4j-cypher --version 1.0.0 --set minibridge.basicAuth.value="supersecret"
```
...
|-----------|------|-------------|-----------|
| sample_size | integer | The sample size used to infer the graph schema. Larger samples are slower, but more accurate. Smaller samples are faster, but might miss information. | No
</details>
<details>
...
| Name | Type | Description | Required? |
|-----------|------|-------------|-----------|
| params | object | The parameters to pass to the Cypher query. | No
...
|-----------|------|------|------|
| tools | get_neo4j_schema | description | f3ada5d5cde18f6e5124f6853354aa15df3460bb4f294337dea236eda5e9748b |
| tools | get_neo4j_schema | sample_size | 24dd535e561799794239015760ed4933e54d21d8e998ce2201a93ece6c472b1d |
