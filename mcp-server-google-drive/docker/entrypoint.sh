...
[ -z "$GDRIVE_CREDENTIALS_PATH" ] && echo "!!! Error mcp-server-google-drive requires GDRIVE_CREDENTIALS_PATH env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-gdrive "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-gdrive
fi
