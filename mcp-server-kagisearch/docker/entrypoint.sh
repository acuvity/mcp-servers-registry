#!/bin/sh
. .venv/bin/activate
[ -z "$KAGI_API_KEY" ] && echo "!!! Error mcp-server-kagisearch requires KAGI_API_KEY env var to be set." && exit 1
[ -z "$KAGI_SUMMARIZER_ENGINE" ] && echo "!!! Error mcp-server-kagisearch requires KAGI_SUMMARIZER_ENGINE env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- kagimcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- kagimcp
fi
