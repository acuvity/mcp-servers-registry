...
[ -z "$MOBSF_URL" ] && echo "!!! Error mcp-server-mobsf requires MOBSF_URL env var to be set." && exit 1
[ -z "$MOBSF_API_KEY" ] && echo "!!! Error mcp-server-mobsf requires MOBSF_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- nkcc-apk-mobsf "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- nkcc-apk-mobsf
fi
