...
[ -z "$APIMATIC_API_KEY" ] && echo "!!! Error mcp-server-apimatic-validator requires APIMATIC_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  ...
  exec minibridge ${MINIBRIDGE_MODE} -- node build/index.js
fi
