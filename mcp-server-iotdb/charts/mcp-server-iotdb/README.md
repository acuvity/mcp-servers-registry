...
The [ARC](https://github.com/acuvity/mcp-servers-registry/tree/main) container includes a [built-in Rego policy](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-iotdb/docker/policy.rego) that enables a set of runtime [guardrails](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-iotdb#%EF%B8%8F-guardrails) to help enforce security, privacy, and correct usage of your services. Below is list of each guardrail provided.
...
**Maintained by**:
  ...
  - [ apache ](https://github.com/apache/iotdb-mcp-server) for application
...
**Current supported version:**
  - charts: `1.0.0`
  - container: `1.0.0-cf77711`

**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - charts: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-iotdb:1.0.0`
  - container: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-iotdb:1.0.0-cf77711`
...
```console
helm show chart oci://docker.io/acuvity/mcp-server-iotdb --version 1.0.0
````
...
```console
helm upgrade mcp-server-iotdb oci://docker.io/acuvity/mcp-server-iotdb --version 1.0.0
```

Uninstall with helm

```console
helm uninstall mcp-server-iotdb
```

From there your MCP server mcp-server-iotdb will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-iotdb` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-iotdb
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-iotdb` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
...
```console
helm upgrade mcp-server-iotdb oci://docker.io/acuvity/mcp-server-iotdb --version 1.0.0 --set 'minibridge.guardrails={secrets-redaction}'
```
...
```console
helm upgrade mcp-server-iotdb oci://docker.io/acuvity/mcp-server-iotdb --version 1.0.0 -f values.yaml
```

To enable basic auth:

```console
helm upgrade mcp-server-iotdb oci://docker.io/acuvity/mcp-server-iotdb --version 1.0.0 --set minibridge.basicAuth.value="supersecret"
```
