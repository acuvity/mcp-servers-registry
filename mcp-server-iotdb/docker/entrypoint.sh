#!/bin/sh
. .venv/bin/activate
[ -z "$IOTDB_HOST" ] && echo "!!! Error mcp-server-iotdb requires IOTDB_HOST env var to be set." && exit 1
export IOTDB_PORT=${IOTDB_PORT:-"6667"}
[ -z "$IOTDB_USER" ] && echo "!!! Error mcp-server-iotdb requires IOTDB_USER env var to be set." && exit 1
[ -z "$IOTDB_PASSWORD" ] && echo "!!! Error mcp-server-iotdb requires IOTDB_PASSWORD env var to be set." && exit 1
[ -z "$IOTDB_DATABASE" ] && echo "!!! Error mcp-server-iotdb requires IOTDB_DATABASE env var to be set." && exit 1
export IOTDB_SQL_DIALECT=${IOTDB_SQL_DIALECT:-"table"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- iotdb-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- iotdb-mcp-server
fi
