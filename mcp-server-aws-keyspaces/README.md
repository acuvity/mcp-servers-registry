    ...
    "servers": {
      "acuvity-mcp-server-aws-keyspaces": {
        ...
        "args": [
          ...
          "docker.io/acuvity/mcp-server-aws-keyspaces:0.0.12"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        ...
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","-e","DB_CASSANDRA_USERNAME","-e","DB_CASSANDRA_PASSWORD","docker.io/acuvity/mcp-server-aws-keyspaces:0.0.12"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only -e DB_CASSANDRA_USERNAME -e DB_CASSANDRA_PASSWORD docker.io/acuvity/mcp-server-aws-keyspaces:0.0.12`
...
```console
docker run -it -p 8000:8000 --rm --read-only -e DB_CASSANDRA_USERNAME -e DB_CASSANDRA_PASSWORD docker.io/acuvity/mcp-server-aws-keyspaces:0.0.12
```
