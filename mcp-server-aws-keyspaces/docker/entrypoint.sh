#!/bin/sh
. .venv/bin/activate
export DB_USE_KEYSPACES=${DB_USE_KEYSPACES:-"true"}
[ -z "$DB_CASSANDRA_USERNAME" ] && echo "!!! Error mcp-server-aws-keyspaces requires DB_CASSANDRA_USERNAME env var to be set." && exit 1
[ -z "$DB_CASSANDRA_PASSWORD" ] && echo "!!! Error mcp-server-aws-keyspaces requires DB_CASSANDRA_PASSWORD env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.amazon-keyspaces-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.amazon-keyspaces-mcp-server
fi
