...
[ -z "$ES_API_KEY" ] && echo "!!! Error mcp-server-elasticsearch requires ES_API_KEY env var to be set." && exit 1
[ -z "$ES_URL" ] && echo "!!! Error mcp-server-elasticsearch requires ES_URL env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-elasticsearch "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-elasticsearch
fi
