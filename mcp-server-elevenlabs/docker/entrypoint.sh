#!/bin/sh
. .venv/bin/activate
[ -z "$ELEVENLABS_API_KEY" ] && echo "!!! Error mcp-server-elevenlabs requires ELEVENLABS_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- elevenlabs-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- elevenlabs-mcp
fi
