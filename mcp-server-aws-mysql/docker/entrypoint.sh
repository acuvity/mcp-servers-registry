...
[ -z "$RESOURCE_ARN" ] && echo "!!! Error mcp-server-aws-mysql requires RESOURCE_ARN env var to be set." && exit 1
[ -z "$SECRET_ARN" ] && echo "!!! Error mcp-server-aws-mysql requires SECRET_ARN env var to be set." && exit 1
[ -z "$DATABASE_NAME" ] && echo "!!! Error mcp-server-aws-mysql requires DATABASE_NAME env var to be set." && exit 1
[ -z "$DATABASE_REGION" ] && echo "!!! Error mcp-server-aws-mysql requires DATABASE_REGION env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.mysql-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.mysql-mcp-server --resource_arn $RESOURCE_ARN --secret_arn $SECRET_ARN --database $DATABASE_NAME$ --region $DATABASE_REGION --readonly $READ_ONLY
fi
