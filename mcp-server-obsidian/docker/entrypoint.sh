#!/bin/sh
. .venv/bin/activate
[ -z "$OBSIDIAN_API_KEY" ] && echo "!!! Error mcp-server-obsidian requires OBSIDIAN_API_KEY env var to be set." && exit 1
[ -z "$OBSIDIAN_HOST" ] && echo "!!! Error mcp-server-obsidian requires OBSIDIAN_HOST env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-obsidian "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-obsidian
fi
