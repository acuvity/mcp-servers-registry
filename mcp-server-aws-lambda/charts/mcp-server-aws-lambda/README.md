...
**Maintained by**:
  ...
  - [ AWSLabs MCP <203918161+awslabs-mcp@users.noreply.github.com> ](https://github.com/awslabs/mcp/tree/main/src/lambda-mcp-server) for application
...
**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - charts: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-aws-lambda:1.0.0`
  - container: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-aws-lambda:1.0.0-0.1.6`
...
```console
helm upgrade mcp-server-aws-lambda oci://docker.io/acuvity/mcp-server-aws-lambda --version 1.0.0
```

Uninstall with helm

```console
helm uninstall mcp-server-aws-lambda
```

From there your MCP server mcp-server-aws-lambda will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-aws-lambda` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-aws-lambda
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-aws-lambda` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
...
```console
helm upgrade mcp-server-aws-lambda oci://docker.io/acuvity/mcp-server-aws-lambda --version 1.0.0 --set 'minibridge.guardrails={secrets-redaction}'
```
...
```console
helm upgrade mcp-server-aws-lambda oci://docker.io/acuvity/mcp-server-aws-lambda --version 1.0.0 -f values.yaml
```

To enable basic auth:

```console
helm upgrade mcp-server-aws-lambda oci://docker.io/acuvity/mcp-server-aws-lambda --version 1.0.0 --set minibridge.basicAuth.value="supersecret"
```
