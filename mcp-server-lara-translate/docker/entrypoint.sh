...
[ -z "$LARA_ACCESS_KEY_ID" ] && echo "!!! Error mcp-server-lara-translate requires LARA_ACCESS_KEY_ID env var to be set." && exit 1
[ -z "$LARA_ACCESS_KEY_SECRET" ] && echo "!!! Error mcp-server-lara-translate requires LARA_ACCESS_KEY_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- lara-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- lara-mcp
fi
