...
[ -z "$GCP_PROJECT_ID" ] && echo "!!! Error mcp-server-google-bigquery requires GCP_PROJECT_ID env var to be set." && exit 1
[ -z "$GCP_REGION" ] && echo "!!! Error mcp-server-google-bigquery requires GCP_REGION env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-bigquery "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-bigquery --project-id $GCP_PROJECT_ID --location $GCP_REGION
fi
