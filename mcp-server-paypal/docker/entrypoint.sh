...
[ -z "$PAYPAL_ACCESS_TOKEN" ] && echo "!!! Error mcp-server-paypal requires PAYPAL_ACCESS_TOKEN env var to be set." && exit 1
export PAYPAL_ENVIRONMENT=${PAYPAL_ENVIRONMENT:-"SANDBOX"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp --tools=all
fi
