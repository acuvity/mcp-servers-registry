#!/bin/sh
. .venv/bin/activate
[ -z "$CHRONULUS_API_KEY" ] && echo "!!! Error mcp-server-chronulus-ai requires CHRONULUS_API_KEY env var to be set." && exit 1
...
exec minibridge ${MINIBRIDGE_MODE} -- chronulus-mcp "$@"
