...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-playwright "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-playwright --headless --browser chromium --executable-path $(ls -d /app/ms-playwright/chromium-* | head -n 1)/chrome-linux/chrome --no-sandbox
fi
