...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.aws-serverless-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.aws-serverless-mcp-server --allow-write --allow-sensitive-data-access
fi
