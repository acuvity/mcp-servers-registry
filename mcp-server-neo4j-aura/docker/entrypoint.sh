#!/bin/sh
. .venv/bin/activate
[ -z "$NEO4J_AURA_CLIENT_ID" ] && echo "!!! Error mcp-server-neo4j-aura requires NEO4J_AURA_CLIENT_ID env var to be set." && exit 1
[ -z "$NEO4J_AURA_CLIENT_SECRET" ] && echo "!!! Error mcp-server-neo4j-aura requires NEO4J_AURA_CLIENT_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-neo4j-aura-manager "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-neo4j-aura-manager
fi
