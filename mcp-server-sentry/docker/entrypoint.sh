#!/bin/sh
. .venv/bin/activate
[ -z "$SENTRY_TOKEN" ] && echo "!!! Error mcp-server-sentry requires SENTRY_TOKEN env var to be set." && exit 1
export SENTRY_API_BASE=${SENTRY_API_BASE:-"https://sentry.io/api/0/"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-sentry "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-sentry
fi
