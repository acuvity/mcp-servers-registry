#!/bin/sh
[ -z "$HARNESS_API_KEY" ] && echo "!!! Error mcp-server-harness requires HARNESS_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- harness-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- harness-mcp-server stdio
fi
