#!/bin/sh
. .venv/bin/activate
[ -z "$DEVHUB_API_KEY" ] && echo "!!! Error mcp-server-devhub requires DEVHUB_API_KEY env var to be set." && exit 1
[ -z "$DEVHUB_API_SECRET" ] && echo "!!! Error mcp-server-devhub requires DEVHUB_API_SECRET env var to be set." && exit 1
[ -z "$DEVHUB_BASE_URL" ] && echo "!!! Error mcp-server-devhub requires DEVHUB_BASE_URL env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- devhub-cms-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- devhub-cms-mcp
fi
