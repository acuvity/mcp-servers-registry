...
[ -z "$BROWSERSTACK_USERNAME" ] && echo "!!! Error mcp-server-browserstack requires BROWSERSTACK_USERNAME env var to be set." && exit 1
[ -z "$BROWSERSTACK_ACCESS_KEY" ] && echo "!!! Error mcp-server-browserstack requires BROWSERSTACK_ACCESS_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- browserstack-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- browserstack-mcp-server
fi
