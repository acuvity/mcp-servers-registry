#!/bin/sh
. .venv/bin/activate
[ -z "$AIRFLOW_HOST" ] && echo "!!! Error mcp-server-apache-airflow requires AIRFLOW_HOST env var to be set." && exit 1
[ -z "$AIRFLOW_PASSWORD" ] && echo "!!! Error mcp-server-apache-airflow requires AIRFLOW_PASSWORD env var to be set." && exit 1
[ -z "$AIRFLOW_USERNAME" ] && echo "!!! Error mcp-server-apache-airflow requires AIRFLOW_USERNAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-apache-airflow "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-apache-airflow
fi
