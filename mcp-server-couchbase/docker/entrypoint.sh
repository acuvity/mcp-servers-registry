#!/bin/sh
. .venv/bin/activate
[ -z "$CB_CONNECTION_STRING" ] && echo "!!! Error mcp-server-couchbase requires CB_CONNECTION_STRING env var to be set." && exit 1
[ -z "$CB_USERNAME" ] && echo "!!! Error mcp-server-couchbase requires CB_USERNAME env var to be set." && exit 1
[ -z "$CB_PASSWORD" ] && echo "!!! Error mcp-server-couchbase requires CB_PASSWORD env var to be set." && exit 1
[ -z "$CB_BUCKET_NAME" ] && echo "!!! Error mcp-server-couchbase requires CB_BUCKET_NAME env var to be set." && exit 1
export READ_ONLY_QUERY_MODE=${READ_ONLY_QUERY_MODE:-"true"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-couchbase "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-couchbase
fi
