...
> [!TIP]
> See [Docker Hub Tags](https://hub.docker.com/r/acuvity/mcp-server-sanity-storage/tags) section for older tags.

# 📦 How to Use


> [!NOTE]
> Given mcp-server-sanity-storage scope of operation it can be hosted anywhere.
...
- command: `docker`
- arguments: `run -i --rm --read-only -v memory:/data docker.io/acuvity/mcp-server-sanity-storage:2025.4.8`
...
```console
docker run -i --rm --read-only -v memory:/data docker.io/acuvity/mcp-server-sanity-storage:2025.4.8
```
...
{
  "mcpServers": {
    "acuvity-mcp-server-sanity-storage": {
...
```console
helm install mcp-server-sanity-storage oci://docker.io/acuvity/mcp-server-sanity-storage --version <version>
```

From there your MCP server mcp-server-sanity-storage will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-sanity-storage` on port `8000` by default. You can change that by looking at the `service` section of the `values.yaml` file.
...
```json
{
  "mcp": {
    "servers": {
      "acuvity-mcp-server-sanity-storage": {
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","-v","memory:/data","docker.io/acuvity/mcp-server-sanity-storage:2025.4.8"]
      }
    }
  }
}
...
> [!NOTE]
> For detailed list of all features, arguments and SBOM hashes provided by this tool please consult the [readme](https://github.com/acuvity/mcp-servers-registry/mcp-server-sanity-storage)
