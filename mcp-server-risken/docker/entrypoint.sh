#!/bin/sh
[ -z "$RISKEN_ACCESS_TOKEN" ] && echo "!!! Error mcp-server-risken requires RISKEN_ACCESS_TOKEN env var to be set." && exit 1
export RISKEN_URL=${RISKEN_URL:-"http://localhost:8000"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- risken-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- risken-mcp-server stdio
fi
