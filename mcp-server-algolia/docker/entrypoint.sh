...
[ -z "$ALGOLIA_CREDENTIALS" ] && echo "!!! Error mcp-server-algolia requires ALGOLIA_CREDENTIALS env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- node "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- node /app/build/app.ts --credentials $ALGOLIA_CREDENTIALS
fi
