...
[ -z "$AUDIENSE_CLIENT_ID" ] && echo "!!! Error mcp-server-audiense-insights requires AUDIENSE_CLIENT_ID env var to be set." && exit 1
[ -z "$AUDIENSE_CLIENT_SECRET" ] && echo "!!! Error mcp-server-audiense-insights requires AUDIENSE_CLIENT_SECRET env var to be set." && exit 1
[ -z "$TWITTER_BEARER_TOKEN" ] && echo "!!! Error mcp-server-audiense-insights requires TWITTER_BEARER_TOKEN env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- audiense-insights "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- audiense-insights
fi
