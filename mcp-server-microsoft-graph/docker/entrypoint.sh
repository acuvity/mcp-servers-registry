#!/bin/sh
[ -z "$MCP_SERVER_MICROSOFT_GRAPH_TENANT_ID" ] && echo "!!! Error mcp-server-microsoft-graph requires MCP_SERVER_MICROSOFT_GRAPH_TENANT_ID env var to be set." && exit 1
[ -z "$MCP_SERVER_MICROSOFT_GRAPH_CLIENT_ID" ] && echo "!!! Error mcp-server-microsoft-graph requires MCP_SERVER_MICROSOFT_GRAPH_CLIENT_ID env var to be set." && exit 1
[ -z "$MCP_SERVER_MICROSOFT_GRAPH_CLIENT_SECRET" ] && echo "!!! Error mcp-server-microsoft-graph requires MCP_SERVER_MICROSOFT_GRAPH_CLIENT_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-microsoft-graph "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-microsoft-graph --transport stdio
fi
