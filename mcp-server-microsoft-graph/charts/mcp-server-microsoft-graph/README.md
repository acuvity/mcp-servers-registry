...
The [ARC](https://github.com/acuvity/mcp-servers-registry/tree/main) container includes a [built-in Rego policy](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-microsoft-graph/docker/policy.rego) that enables a set of runtime [guardrails](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-microsoft-graph#%EF%B8%8F-guardrails) to help enforce security, privacy, and correct usage of your services. Below is list of each guardrail provided.
...
**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - charts: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-microsoft-graph:1.0.0`
  - container: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-microsoft-graph:1.0.0-1.0.1`
...
```console
helm show chart oci://docker.io/acuvity/mcp-server-microsoft-graph --version 1.0.0
````
...
```console
helm upgrade mcp-server-microsoft-graph oci://docker.io/acuvity/mcp-server-microsoft-graph --version 1.0.0
```

Uninstall with helm

```console
helm uninstall mcp-server-microsoft-graph
```

From there your MCP server mcp-server-microsoft-graph will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-microsoft-graph` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-microsoft-graph
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-microsoft-graph` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
...
```console
helm upgrade mcp-server-microsoft-graph oci://docker.io/acuvity/mcp-server-microsoft-graph --version 1.0.0 --set 'minibridge.guardrails={secrets-redaction}'
```
...
```console
helm upgrade mcp-server-microsoft-graph oci://docker.io/acuvity/mcp-server-microsoft-graph --version 1.0.0 -f values.yaml
```

To enable basic auth:

```console
helm upgrade mcp-server-microsoft-graph oci://docker.io/acuvity/mcp-server-microsoft-graph --version 1.0.0 --set minibridge.basicAuth.value="supersecret"
```
