...
[ -z "$FIGMA_API_KEY" ] && echo "!!! Error mcp-server-figma requires FIGMA_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- figma-developer-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- figma-developer-mcp --stdio
fi
