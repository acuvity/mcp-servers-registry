#!/bin/sh
. .venv/bin/activate
[ -z "$KENDRA_INDEX_ID" ] && echo "!!! Error mcp-server-aws-kendra-index requires KENDRA_INDEX_ID env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.amazon-kendra-index-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.amazon-kendra-index-mcp-server
fi
