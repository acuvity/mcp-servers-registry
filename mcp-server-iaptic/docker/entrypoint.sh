...
[ -z "$IAPTIC_API_KEY" ] && echo "!!! Error mcp-server-iaptic requires IAPTIC_API_KEY env var to be set." && exit 1
[ -z "$IAPTIC_APP_NAME" ] && echo "!!! Error mcp-server-iaptic requires IAPTIC_APP_NAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-iaptic "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-iaptic
fi
