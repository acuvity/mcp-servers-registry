...
[![Docker](https://img.shields.io/docker/image-size/acuvity/mcp-server-clickhouse/0.1.13?logo=docker&logoColor=fff&label=0.1.13)](https://hub.docker.com/r/acuvity/mcp-server-clickhouse)
[![PyPI](https://img.shields.io/badge/0.1.13-3775A9?logo=pypi&logoColor=fff&label=mcp-clickhouse)](https://github.com/ClickHouse/mcp-clickhouse)
...
[![Install in VS Code Docker](https://img.shields.io/badge/VS_Code-One_click_install-0078d7?logo=githubcopilot)](https://insiders.vscode.dev/redirect/mcp/install?name=mcp-server-clickhouse&config=%7B%22args%22%3A%5B%22run%22%2C%22-i%22%2C%22--rm%22%2C%22--read-only%22%2C%22--tmpfs%22%2C%22%2Ftmp%3Arw%2Cnosuid%2Cnodev%22%2C%22-e%22%2C%22CLICKHOUSE_HOST%22%2C%22-e%22%2C%22CLICKHOUSE_USER%22%2C%22-e%22%2C%22CLICKHOUSE_PASSWORD%22%2C%22docker.io%2Facuvity%2Fmcp-server-clickhouse%3A0.1.13%22%5D%2C%22command%22%3A%22docker%22%7D)
...
The [ARC](https://github.com/acuvity/mcp-servers-registry/tree/main) container includes a [built-in Rego policy](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-clickhouse/docker/policy.rego) that enables a set of runtime [guardrails](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-clickhouse#%EF%B8%8F-guardrails) to help enforce security, privacy, and correct usage of your services. Below is list of each guardrail provided.
...
**Maintained by**:
  ...
  - [ Author ](https://github.com/ClickHouse/mcp-clickhouse) for application
...
**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - charts: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-clickhouse:1.0.0`
  - container: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-clickhouse:1.0.0-0.1.13`
...
```console
helm show chart oci://docker.io/acuvity/mcp-server-clickhouse --version 1.0.0
````
...
```console
helm upgrade mcp-server-clickhouse oci://docker.io/acuvity/mcp-server-clickhouse --version 1.0.0
```

Uninstall with helm

```console
helm uninstall mcp-server-clickhouse
```

From there your MCP server mcp-server-clickhouse will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-clickhouse` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-clickhouse
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-clickhouse` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
...
```console
helm upgrade mcp-server-clickhouse oci://docker.io/acuvity/mcp-server-clickhouse --version 1.0.0 --set 'minibridge.guardrails={secrets-redaction}'
```
...
```console
helm upgrade mcp-server-clickhouse oci://docker.io/acuvity/mcp-server-clickhouse --version 1.0.0 -f values.yaml
```

To enable basic auth:

```console
helm upgrade mcp-server-clickhouse oci://docker.io/acuvity/mcp-server-clickhouse --version 1.0.0 --set minibridge.basicAuth.value="supersecret"
```
...
| include_detailed_columns | boolean | not set | No
| like | any | not set | No
| not_like | any | not set | No
| page_size | integer | not set | No
| page_token | any | not set | No
</details>
<details>
...
| tools | list_tables | description | 7ae50e8ff1ab6a03661b9996cdbd3121a15ca46e2b27c45379664221ceb5a7ce |
