#!/bin/sh
. .venv/bin/activate
[ -z "$ADB_MYSQL_HOST" ] && echo "!!! Error mcp-server-alibabacloud-adb-mysql requires ADB_MYSQL_HOST env var to be set." && exit 1
[ -z "$ADB_MYSQL_USER" ] && echo "!!! Error mcp-server-alibabacloud-adb-mysql requires ADB_MYSQL_USER env var to be set." && exit 1
[ -z "$ADB_MYSQL_PASSWORD" ] && echo "!!! Error mcp-server-alibabacloud-adb-mysql requires ADB_MYSQL_PASSWORD env var to be set." && exit 1
[ -z "$ADB_MYSQL_DATABASE" ] && echo "!!! Error mcp-server-alibabacloud-adb-mysql requires ADB_MYSQL_DATABASE env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- adb-mysql-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- adb-mysql-mcp-server
fi
