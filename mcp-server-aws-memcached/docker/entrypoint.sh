#!/bin/sh
. .venv/bin/activate
[ -z "$MEMCACHED_HOST" ] && echo "!!! Error mcp-server-aws-memcached requires MEMCACHED_HOST env var to be set." && exit 1
export MEMCACHED_PORT=${MEMCACHED_PORT:-"11211"}
export MEMCACHED_TIMEOUT=${MEMCACHED_TIMEOUT:-"1"}
export MEMCACHED_CONNECT_TIMEOUT=${MEMCACHED_CONNECT_TIMEOUT:-"5"}
export MEMCACHED_RETRY_TIMEOUT=${MEMCACHED_RETRY_TIMEOUT:-"1"}
export MEMCACHED_MAX_RETRIES=${MEMCACHED_MAX_RETRIES:-"3"}
export MEMCACHED_TLS_VERIFY=${MEMCACHED_TLS_VERIFY:-"true"}
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.memcached-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.memcached-mcp-server
fi
