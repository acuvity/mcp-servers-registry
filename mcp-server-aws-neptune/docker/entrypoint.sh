#!/bin/sh
. .venv/bin/activate
[ -z "$NEPTUNE_ENDPOINT" ] && echo "!!! Error mcp-server-aws-neptune requires NEPTUNE_ENDPOINT env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.amazon-neptune-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.amazon-neptune-mcp-server
fi
