#!/bin/sh
[ -z "$ASGARDEO_BASE_URL" ] && echo "!!! Error mcp-server-asgardeo requires ASGARDEO_BASE_URL env var to be set." && exit 1
[ -z "$ASGARDEO_CLIENT_ID" ] && echo "!!! Error mcp-server-asgardeo requires ASGARDEO_CLIENT_ID env var to be set." && exit 1
[ -z "$ASGARDEO_CLIENT_SECRET" ] && echo "!!! Error mcp-server-asgardeo requires ASGARDEO_CLIENT_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- asgardeo-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- asgardeo-mcp
fi
