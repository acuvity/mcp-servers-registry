#!/bin/sh
[ -z "$AXIOM_TOKEN" ] && echo "!!! Error mcp-server-axiom requires AXIOM_TOKEN env var to be set." && exit 1
[ -z "$AXIOM_URL" ] && echo "!!! Error mcp-server-axiom requires AXIOM_URL env var to be set." && exit 1
[ -z "$AXIOM_ORG" ] && echo "!!! Error mcp-server-axiom requires AXIOM_ORG env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- axiom-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- axiom-mcp
fi
