#!/bin/sh
. .venv/bin/activate
[ -z "$NEO4J_PASSWORD" ] && echo "!!! Error mcp-server-neo4j-memory requires NEO4J_PASSWORD env var to be set." && exit 1
[ -z "$NEO4J_URL" ] && echo "!!! Error mcp-server-neo4j-memory requires NEO4J_URL env var to be set." && exit 1
[ -z "$NEO4J_USERNAME" ] && echo "!!! Error mcp-server-neo4j-memory requires NEO4J_USERNAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-neo4j-memory "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-neo4j-memory
fi
