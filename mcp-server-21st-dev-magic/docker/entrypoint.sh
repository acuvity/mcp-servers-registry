...
[ -z "$TWENTY_FIRST_API_KEY" ] && echo "!!! Error mcp-server-21st-dev-magic requires TWENTY_FIRST_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- magic "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- magic
fi
