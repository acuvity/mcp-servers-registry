...
[ -z "$API_KEY" ] && echo "!!! Error mcp-server-coda requires API_KEY env var to be set." && exit 1
[ -z "$DOC_ID" ] && echo "!!! Error mcp-server-coda requires DOC_ID env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- coda "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- coda
fi
