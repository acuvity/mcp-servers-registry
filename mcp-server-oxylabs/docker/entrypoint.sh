#!/bin/sh
. .venv/bin/activate
[ -z "$OXYLABS_PASSWORD" ] && echo "!!! Error mcp-server-oxylabs requires OXYLABS_PASSWORD env var to be set." && exit 1
[ -z "$OXYLABS_USERNAME" ] && echo "!!! Error mcp-server-oxylabs requires OXYLABS_USERNAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- oxylabs-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- oxylabs-mcp
fi
