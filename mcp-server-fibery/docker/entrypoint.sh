#!/bin/sh
. .venv/bin/activate
[ -z "$FIBERY_API_TOKEN" ] && echo "!!! Error mcp-server-fibery requires FIBERY_API_TOKEN env var to be set." && exit 1
[ -z "$FIBERY_HOST" ] && echo "!!! Error mcp-server-fibery requires FIBERY_HOST env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- fibery-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- fibery-mcp-server
fi
