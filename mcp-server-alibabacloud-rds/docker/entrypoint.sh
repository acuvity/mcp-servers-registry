#!/bin/sh
. .venv/bin/activate
[ -z "$ALIBABA_CLOUD_ACCESS_KEY_ID" ] && echo "!!! Error mcp-server-alibabacloud-rds requires ALIBABA_CLOUD_ACCESS_KEY_ID env var to be set." && exit 1
[ -z "$ALIBABA_CLOUD_ACCESS_KEY_SECRET" ] && echo "!!! Error mcp-server-alibabacloud-rds requires ALIBABA_CLOUD_ACCESS_KEY_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- alibabacloud-rds-openapi-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- alibabacloud-rds-openapi-mcp-server
fi
