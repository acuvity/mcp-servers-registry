    ...
    "servers": {
      "acuvity-mcp-server-chart": {
        ...
        "args": [
          ...
          "--rm",
          "--read-only",
          "docker.io/acuvity/mcp-server-chart:0.2.6"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","docker.io/acuvity/mcp-server-chart:0.2.6"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only docker.io/acuvity/mcp-server-chart:0.2.6`
...
```console
docker run -it -p 8000:8000 --rm --read-only docker.io/acuvity/mcp-server-chart:0.2.6
```
