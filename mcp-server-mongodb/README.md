    ...
    "servers": {
      "acuvity-mcp-server-mongodb": {
        ...
        "args": [
          ...
          "--tmpfs",
          "/tmp:rw,nosuid,nodev",
          "docker.io/acuvity/mcp-server-mongodb:1.5.0"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","--tmpfs","/tmp:rw,nosuid,nodev","docker.io/acuvity/mcp-server-mongodb:1.5.0"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only --tmpfs /tmp:rw,nosuid,nodev docker.io/acuvity/mcp-server-mongodb:1.5.0`
...
```console
docker run -it -p 8000:8000 --rm --read-only --tmpfs /tmp:rw,nosuid,nodev docker.io/acuvity/mcp-server-mongodb:1.5.0
```
