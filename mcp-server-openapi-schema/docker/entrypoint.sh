...
[ -z "$SCHEMA_PATH" ] && echo "!!! Error mcp-server-openapi-schema requires SCHEMA_PATH env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-openapi-schema "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-openapi-schema $SCHEMA_PATH
fi
