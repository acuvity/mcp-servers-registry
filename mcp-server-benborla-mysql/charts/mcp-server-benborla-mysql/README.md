...
**Maintained by**:
  ...
  - [ Ben Borla ](https://github.com/benborla/mcp-server-mysql) for application
...
**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - charts: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-benborla-mysql:1.0.0`
  - container: `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-benborla-mysql:1.0.0-2.0.2`
...
```console
helm upgrade mcp-server-benborla-mysql oci://docker.io/acuvity/mcp-server-benborla-mysql --version 1.0.0
```

Uninstall with helm

```console
helm uninstall mcp-server-benborla-mysql
```

From there your MCP server mcp-server-benborla-mysql will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-benborla-mysql` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-benborla-mysql
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-benborla-mysql` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
...
```console
helm upgrade mcp-server-benborla-mysql oci://docker.io/acuvity/mcp-server-benborla-mysql --version 1.0.0 --set 'minibridge.guardrails={secrets-redaction}'
```
...
```console
helm upgrade mcp-server-benborla-mysql oci://docker.io/acuvity/mcp-server-benborla-mysql --version 1.0.0 -f values.yaml
```

To enable basic auth:

```console
helm upgrade mcp-server-benborla-mysql oci://docker.io/acuvity/mcp-server-benborla-mysql --version 1.0.0 --set minibridge.basicAuth.value="supersecret"
```
...
```
[MySQL MCP Server [vundefined]] Run SQL queries against MySQL database with support for: INSERT, UPDATE, DELETE and READ operations
```
...
|-----------|------|------|------|
| tools | mysql_query | description | 0a73faebde1536c76227d8bb98021ae75480e186c0d0ded95b7a4f86fe52c994 |
