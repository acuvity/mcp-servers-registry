#!/bin/sh
. .venv/bin/activate
[ -z "$ADFIN_EMAIL" ] && echo "!!! Error mcp-server-adfin requires ADFIN_EMAIL env var to be set." && exit 1
[ -z "$ADFIN_PASSWORD" ] && echo "!!! Error mcp-server-adfin requires ADFIN_PASSWORD env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- main_adfin_mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- main_adfin_mcp
fi
