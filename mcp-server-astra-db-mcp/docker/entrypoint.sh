...
[ -z "$ASTRA_DB_API_ENDPOINT" ] && echo "!!! Error mcp-server-astra-db-mcp requires ASTRA_DB_API_ENDPOINT env var to be set." && exit 1
[ -z "$ASTRA_DB_APPLICATION_TOKEN" ] && echo "!!! Error mcp-server-astra-db-mcp requires ASTRA_DB_APPLICATION_TOKEN env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- astra-db-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- astra-db-mcp
fi
