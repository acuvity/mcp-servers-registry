...
[ -z "$HYPERBROWSER_API_KEY" ] && echo "!!! Error mcp-server-hyperbrowser requires HYPERBROWSER_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- hyperbrowser-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- hyperbrowser-mcp
fi
