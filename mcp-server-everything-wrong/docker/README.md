...
**Maintained by**:
  ...
  - [ Author ](https://pypi.org/project/mcp-server-everything-wrong/) for original source application
...
**Latest tags:**
  ...
  - [older tags](https://hub.docker.com/r/acuvity/mcp-server-everything-wrong/tags)

**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-everything-wrong:latest`
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-everything-wrong:0.2.1`
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-everything-wrong:1.0.0-0.2.1`

# 📦 How to Install


> [!TIP]
> Given mcp-server-everything-wrong scope of operation the intended usage is to run natively on the targeted machine to access local resources.

For more information and extra configuration you can consult the [package](https://pypi.org/project/mcp-server-everything-wrong/) documentation.
    ...
    "servers": {
      "acuvity-mcp-server-everything-wrong": {
        ...
        "args": [
          ...
          "--rm",
          "--read-only",
          "docker.io/acuvity/mcp-server-everything-wrong:0.2.1"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","docker.io/acuvity/mcp-server-everything-wrong:0.2.1"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only docker.io/acuvity/mcp-server-everything-wrong:0.2.1`
...
```console
docker run -it -p 8000:8000 --rm --read-only docker.io/acuvity/mcp-server-everything-wrong:0.2.1
```
...
```console
helm show readme oci://docker.io/acuvity/mcp-server-everything-wrong --version 1.0.0
````
...
From there your MCP server mcp-server-everything-wrong will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-everything-wrong` on port `8000` by default. You can change that by looking at the `service` section of the `values.yaml` file.

### How to Monitor

The deployment will create a Kubernetes service with a `healthPort`, that is used for liveness probes and readiness probes. This health port can also be used by the monitoring stack of your choice and exposes metrics under the `/metrics` path.

See full charts [Readme](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-everything-wrong/charts/mcp-server-everything-wrong/README.md) for more details about settings and runtime security including guardrails activation.

</details>

# 🧠 Server features

For detailed list of all features, tools, arguments and SBOM hashes provided by this server please consult the [readme](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-everything-wrong)
