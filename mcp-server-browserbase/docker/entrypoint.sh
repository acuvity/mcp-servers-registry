...
[ -z "$BROWSERBASE_API_KEY" ] && echo "!!! Error mcp-server-browserbase requires BROWSERBASE_API_KEY env var to be set." && exit 1
[ -z "$BROWSERBASE_PROJECT_ID" ] && echo "!!! Error mcp-server-browserbase requires BROWSERBASE_PROJECT_ID env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-browserbase "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-browserbase
fi
