    ...
    "servers": {
      "acuvity-mcp-server-azure": {
        ...
        "args": [
          ...
          "-e",
          "AZURE_TENANT_ID",
          "docker.io/acuvity/mcp-server-azure:0.0.18"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        ...
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","--tmpfs","/tmp:rw,nosuid,nodev","-e","AZURE_CLIENT_ID","-e","AZURE_CLIENT_SECRET","-e","AZURE_TENANT_ID","docker.io/acuvity/mcp-server-azure:0.0.18"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only --tmpfs /tmp:rw,nosuid,nodev -e AZURE_CLIENT_ID -e AZURE_CLIENT_SECRET -e AZURE_TENANT_ID docker.io/acuvity/mcp-server-azure:0.0.18`
...
```console
docker run -it -p 8000:8000 --rm --read-only --tmpfs /tmp:rw,nosuid,nodev -e AZURE_CLIENT_ID -e AZURE_CLIENT_SECRET -e AZURE_TENANT_ID docker.io/acuvity/mcp-server-azure:0.0.18
```
