#!/bin/sh
. .venv/bin/activate
[ -z "$CHROMA_CLIENT_TYPE" ] && echo "!!! Error mcp-server-chroma requires CHROMA_CLIENT_TYPE env var to be set." && exit 1
[ -z "$CHROMA_DATABASE" ] && echo "!!! Error mcp-server-chroma requires CHROMA_DATABASE env var to be set." && exit 1
[ -z "$CHROMA_HOST" ] && echo "!!! Error mcp-server-chroma requires CHROMA_HOST env var to be set." && exit 1
[ -z "$CHROMA_PORT" ] && echo "!!! Error mcp-server-chroma requires CHROMA_PORT env var to be set." && exit 1
[ -z "$CHROMA_TENANT" ] && echo "!!! Error mcp-server-chroma requires CHROMA_TENANT env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- chroma-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- chroma-mcp
fi
