#!/bin/sh
. .venv/bin/activate
[ -z "$HOLOGRES_HOST" ] && echo "!!! Error mcp-server-alibabacloud-hologres requires HOLOGRES_HOST env var to be set." && exit 1
export HOLOGRES_PORT=${HOLOGRES_PORT:-"5432"}
[ -z "$HOLOGRES_PASSWORD" ] && echo "!!! Error mcp-server-alibabacloud-hologres requires HOLOGRES_PASSWORD env var to be set." && exit 1
[ -z "$HOLOGRES_DATABASE" ] && echo "!!! Error mcp-server-alibabacloud-hologres requires HOLOGRES_DATABASE env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- hologres-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- hologres-mcp-server hologres-mcp-server
fi
