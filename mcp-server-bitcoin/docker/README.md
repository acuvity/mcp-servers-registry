...
[![Scout](https://img.shields.io/badge/Active-3775A9?logo=docker&logoColor=fff&label=Scout)](https://hub.docker.com/r/acuvity/mcp-server-bitcoin/)
...
Packaged by Acuvity and published to our curated MCP server [registry](https://mcp.acuvity.ai) from bitcoin-mcp original [sources](https://github.com/AbdelStark/bitcoin-mcp).
...
**Maintained by**:
  ...
  - [ Author ](https://github.com/AbdelStark/bitcoin-mcp) for original source application
...
**Latest tags:**
  - `latest` -> `1.0.0-0.0.6` -> `0.0.6`
  - [older tags](https://hub.docker.com/r/acuvity/mcp-server-bitcoin/tags)

**Verify signature with [cosign](https://github.com/sigstore/cosign):**
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-bitcoin:latest`
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-bitcoin:0.0.6`
  - `cosign verify --certificate-oidc-issuer "https://token.actions.githubusercontent.com" --certificate-identity "https://github.com/acuvity/mcp-servers-registry/.github/workflows/release.yaml@refs/heads/main" docker.io/acuvity/mcp-server-bitcoin:1.0.0-0.0.6`

# 📦 How to Install


> [!TIP]
> Given mcp-server-bitcoin scope of operation it can be hosted anywhere.

For more information and extra configuration you can consult the [package](https://github.com/AbdelStark/bitcoin-mcp) documentation.
    ...
    "servers": {
      "acuvity-mcp-server-bitcoin": {
        ...
        "args": [
          ...
          "--rm",
          "--read-only",
          "docker.io/acuvity/mcp-server-bitcoin:0.0.6"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","docker.io/acuvity/mcp-server-bitcoin:0.0.6"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only docker.io/acuvity/mcp-server-bitcoin:0.0.6`
...
```console
docker run -it -p 8000:8000 --rm --read-only docker.io/acuvity/mcp-server-bitcoin:0.0.6
```
...
```console
helm show readme oci://docker.io/acuvity/mcp-server-bitcoin --version 1.0.0
````
...
From there your MCP server mcp-server-bitcoin will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-bitcoin` on port `8000` by default. You can change that by looking at the `service` section of the `values.yaml` file.

### How to Monitor

The deployment will create a Kubernetes service with a `healthPort`, that is used for liveness probes and readiness probes. This health port can also be used by the monitoring stack of your choice and exposes metrics under the `/metrics` path.

See full charts [Readme](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-bitcoin/charts/mcp-server-bitcoin/README.md) for more details about settings and runtime security including guardrails activation.

</details>

# 🧠 Server features

For detailed list of all features, tools, arguments and SBOM hashes provided by this server please consult the [readme](https://github.com/acuvity/mcp-servers-registry/tree/main/mcp-server-bitcoin)
