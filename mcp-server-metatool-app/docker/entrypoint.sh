...
[ -z "$METAMCP_API_BASE_URL" ] && echo "!!! Error mcp-server-metatool-app requires METAMCP_API_BASE_URL env var to be set." && exit 1
[ -z "$METAMCP_API_KEY" ] && echo "!!! Error mcp-server-metatool-app requires METAMCP_API_KEY env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-metamcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-metamcp
fi
