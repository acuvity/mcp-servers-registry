#!/bin/sh
. .venv/bin/activate
[ -z "$AIVEN_BASE_URL" ] && echo "!!! Error mcp-server-aiven requires AIVEN_BASE_URL env var to be set." && exit 1
[ -z "$AIVEN_TOKEN" ] && echo "!!! Error mcp-server-aiven requires AIVEN_TOKEN env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-aiven "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-aiven
fi
