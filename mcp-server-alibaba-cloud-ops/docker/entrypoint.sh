#!/bin/sh
. .venv/bin/activate
[ -z "$ALIBABA_CLOUD_ACCESS_KEY_ID" ] && echo "!!! Error mcp-server-alibaba-cloud-ops requires ALIBABA_CLOUD_ACCESS_KEY_ID env var to be set." && exit 1
[ -z "$ALIBABA_CLOUD_ACCESS_KEY_SECRET" ] && echo "!!! Error mcp-server-alibaba-cloud-ops requires ALIBABA_CLOUD_ACCESS_KEY_SECRET env var to be set." && exit 1
...
exec minibridge ${MINIBRIDGE_MODE} -- alibaba-cloud-ops-mcp-server "$@"
