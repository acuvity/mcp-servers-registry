#!/bin/sh
. .venv/bin/activate
[ -z "$CONFLUENCE_API_TOKEN" ] && echo "!!! Error mcp-server-atlassian requires CONFLUENCE_API_TOKEN env var to be set." && exit 1
[ -z "$CONFLUENCE_URL" ] && echo "!!! Error mcp-server-atlassian requires CONFLUENCE_URL env var to be set." && exit 1
[ -z "$CONFLUENCE_USERNAME" ] && echo "!!! Error mcp-server-atlassian requires CONFLUENCE_USERNAME env var to be set." && exit 1
[ -z "$JIRA_API_TOKEN" ] && echo "!!! Error mcp-server-atlassian requires JIRA_API_TOKEN env var to be set." && exit 1
[ -z "$JIRA_URL" ] && echo "!!! Error mcp-server-atlassian requires JIRA_URL env var to be set." && exit 1
[ -z "$JIRA_USERNAME" ] && echo "!!! Error mcp-server-atlassian requires JIRA_USERNAME env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-atlassian "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-atlassian
fi
