#!/bin/sh
. .venv/bin/activate
[ -z "$CLUSTER_ENDPOINT" ] && echo "!!! Error mcp-server-aws-aurora-dsql requires CLUSTER_ENDPOINT env var to be set." && exit 1
[ -z "$CLUSTER_REGION" ] && echo "!!! Error mcp-server-aws-aurora-dsql requires CLUSTER_REGION env var to be set." && exit 1
[ -z "$DATABASE_USER" ] && echo "!!! Error mcp-server-aws-aurora-dsql requires DATABASE_USER env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.aurora-dsql-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- awslabs.aurora-dsql-mcp-server --cluster_endpoint $CLUSTER_ENDPOINT --region $CLUSTER_REGION --database_user $DATABASE_USER --allow-writes
fi
