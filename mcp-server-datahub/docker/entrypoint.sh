#!/bin/sh
. .venv/bin/activate
[ -z "$DATAHUB_GMS_URL" ] && echo "!!! Error mcp-server-datahub requires DATAHUB_GMS_URL env var to be set." && exit 1
[ -z "$DATAHUB_GMS_TOKEN" ] && echo "!!! Error mcp-server-datahub requires DATAHUB_GMS_TOKEN env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-datahub "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- mcp-server-datahub
fi
