...
[![Helm](https://img.shields.io/badge/v1.0.0-3775A9?logo=helm&label=Charts&logoColor=fff)](https://hub.docker.com/r/acuvity/mcp-server-sanity-node/tags/)
[![Docker](https://img.shields.io/docker/image-size/acuvity/mcp-server-fetch/2025.4.8?logo=docker&logoColor=fff&label=2025.4.8)](https://hub.docker.com/r/acuvity/mcp-server-sanity-node/tags/2025.4.8)
...
**Resources**:
  - [Charts](https://github.com/acuvity/mcp-servers-registry/mcp-server-sanity-node/charts/mcp-server-sanity-node)
  - [Dockerfile](https://github.com/acuvity/mcp-servers-registry/mcp-server-sanity-node/docker/Dockerfile)
...
```console
helm install helm install mcp-server-sanity-node oci://docker.io/acuvity/mcp-server-sanity-node --version v1.0.0
```

You can inspect the chart:

```console
helm show chart oci://docker.io/acuvity/mcp-server-sanity-node --version v1.0.0
````

You can inpect the values that you can configure:

```console
helm show values oci://docker.io/acuvity/mcp-server-sanity-node --version v1.0.0
````
From there your MCP server mcp-server-sanity-node will be reachable by default through `http/sse` from inside the cluster using the Kubernetes Service `mcp-server-sanity-node` on port `8000` by default.
...
```yaml
image:
  repository: map[host:docker.io org:acuvity]/mcp-server-sanity-node
...
- **storage**: iterates `package.storage` entries:
  - for persistent entries, creates a PersistentVolumeClaim named `mcp-server-sanity-node` with `storageClassName: <class>` and `resources.requests.storage: <size>`.
