...
[ -z "$BANKLESS_API_TOKEN" ] && echo "!!! Error mcp-server-bankless-onchain requires BANKLESS_API_TOKEN env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- onchain-mcp "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- onchain-mcp
fi
