    ...
    "servers": {
      "acuvity-mcp-server-notion": {
        ...
        "args": [
          ...
          "-e",
          "OPENAPI_MCP_HEADERS",
          "docker.io/acuvity/mcp-server-notion:1.8.0"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        ...
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","-e","OPENAPI_MCP_HEADERS","docker.io/acuvity/mcp-server-notion:1.8.0"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only -e OPENAPI_MCP_HEADERS docker.io/acuvity/mcp-server-notion:1.8.0`
...
```console
docker run -it -p 8000:8000 --rm --read-only -e OPENAPI_MCP_HEADERS docker.io/acuvity/mcp-server-notion:1.8.0
```
