    ...
    "servers": {
      "acuvity-mcp-server-tavily": {
        ...
        "args": [
          ...
          "-e",
          "TAVILY_API_KEY",
          "docker.io/acuvity/mcp-server-tavily:0.2.12"
        ]
      }
    }
  }
...
async with MCPServerStdio(
    params={
        ...
        "command": "docker",
        "args": ["run","-i","--rm","--read-only","-e","TAVILY_API_KEY","docker.io/acuvity/mcp-server-tavily:0.2.12"]
    }
) as server:
...
- command: `docker`
- arguments: `run -i --rm --read-only -e TAVILY_API_KEY docker.io/acuvity/mcp-server-tavily:0.2.12`
...
```console
docker run -it -p 8000:8000 --rm --read-only -e TAVILY_API_KEY docker.io/acuvity/mcp-server-tavily:0.2.12
```
