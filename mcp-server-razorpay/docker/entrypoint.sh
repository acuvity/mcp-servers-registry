#!/bin/sh
[ -z "$RAZORPAY_KEY_ID" ] && echo "!!! Error mcp-server-razorpay requires RAZORPAY_KEY_ID env var to be set." && exit 1
[ -z "$RAZORPAY_KEY_SECRET" ] && echo "!!! Error mcp-server-razorpay requires RAZORPAY_KEY_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- razorpay-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- razorpay-mcp-server stdio
fi
