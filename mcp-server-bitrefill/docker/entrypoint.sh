...
[ -z "$BITREFILL_API_ID" ] && echo "!!! Error mcp-server-bitrefill requires BITREFILL_API_ID env var to be set." && exit 1
[ -z "$BITREFILL_API_SECRET" ] && echo "!!! Error mcp-server-bitrefill requires BITREFILL_API_SECRET env var to be set." && exit 1
...
if [ "$#" -gt 0 ]; then
  exec minibridge ${MINIBRIDGE_MODE} -- bitrefill-mcp-server "$@"
else
  exec minibridge ${MINIBRIDGE_MODE} -- bitrefill-mcp-server
fi
